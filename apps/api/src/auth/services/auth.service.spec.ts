...
describe('AuthService', () => {
  ...
  beforeEach(async () => {
    ...
    hashService = module.get<HashService>(HashService) as jest.Mocked<HashService>;
    jwtService = module.get<CustomJwtService>(CustomJwtService) as jest.Mocked<CustomJwtService>;
  });
  ...
  describe('register', () => {
    ...
    beforeEach(() => {
      ...
      mockHashService.hashResetToken.mockResolvedValue('hashed-verification-token');
    });
    ...
    it('should register a new user successfully', async () => {
      ...
      expect(hashService.validatePasswordStrength).toHaveBeenCalledWith('StrongPass123!');
  ...
  describe('login', () => {
    ...
    it('should login user successfully', async () => {
      ...
      expect(hashService.comparePasswords).toHaveBeenCalledWith('StrongPass123!', 'hashed-password');
    ...
    it('should lock account after max failed attempts', async () => {
      ...
      mockPrismaService.user.findUnique.mockResolvedValue(userWithFailedAttempts as any);

      await expect(service.login(loginDto)).rejects.toThrow(
        new ForbiddenException('Too many failed attempts. Account has been locked.')
      );
  ...
  describe('refreshToken', () => {
    ...
    beforeEach(() => {
      ...
      mockHashService.hashResetToken.mockResolvedValue('new-hashed-refresh-token');
      mockJwtService.generateTokenPair.mockResolvedValue(mockNewTokens);
    });

    it('should refresh tokens successfully', async () => {
      mockPrismaService.refreshToken.findMany.mockResolvedValue([mockStoredToken] as any);
      mockPrismaService.user.findUnique.mockResolvedValue(mockUser as any);
    ...
    it('should throw UnauthorizedException for invalid refresh token', async () => {
      mockJwtService.verifyRefreshToken.mockRejectedValue(new UnauthorizedException('Invalid refresh token'));
  ...
  describe('logout', () => {
    ...
    it('should logout and revoke specific refresh token', async () => {
      mockPrismaService.refreshToken.findMany.mockResolvedValue(mockStoredTokens as any);
      mockHashService.verifyResetToken.mockResolvedValueOnce(false).mockResolvedValueOnce(true);
      mockPrismaService.refreshToken.update.mockResolvedValue({} as any);
    ...
    it('should handle logout when no matching token is found', async () => {
      ...
      await expect(service.logout(userId, refreshToken)).resolves.toBeUndefined();
    });
  });
  ...
  describe('verifyEmail', () => {
    ...
    it('should verify email successfully', async () => {
      mockPrismaService.verificationToken.findMany.mockResolvedValue(mockVerificationTokens as any);
      mockHashService.verifyResetToken.mockResolvedValue(true);
      ...
      expect(hashService.verifyResetToken).toHaveBeenCalledWith(token, 'hashed-vtoken1');
  ...
  describe('resetPassword', () => {
    ...
    it('should reset password successfully', async () => {
      mockPrismaService.passwordResetToken.findMany.mockResolvedValue(mockResetTokens as any);
      mockHashService.verifyResetToken.mockResolvedValue(true);
      mockPrismaService.user.update.mockResolvedValue({} as any);
      ...
      expect(hashService.verifyResetToken).toHaveBeenCalledWith('reset-token', 'hashed-rtoken1');
      expect(hashService.validatePasswordStrength).toHaveBeenCalledWith('NewStrongPass123!');
      expect(prismaService.user.update).toHaveBeenCalledWith({
        where: { id: '1' },
  ...
  describe('getUserSessions', () => {
    ...
    it('should get user sessions successfully', async () => {
      mockPrismaService.refreshToken.findMany.mockResolvedValue(mockSessions as any);
  ...
  describe('revokeSession', () => {
    ...
    it('should revoke session successfully', async () => {
      mockPrismaService.refreshToken.findFirst.mockResolvedValue(mockSession as any);
      mockPrismaService.refreshToken.update.mockResolvedValue({} as any);
  ...
  describe('Edge Cases and Security', () => {
    ...
    it('should handle hash service errors gracefully', async () => {
      ...
      mockHashService.hashPassword.mockRejectedValue(new Error('Hashing failed'));
      mockPrismaService.user.findUnique.mockResolvedValue(null);
