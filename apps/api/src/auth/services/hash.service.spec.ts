...
describe('HashService', () => {
  ...
  beforeEach(async () => {
    ...
    mockCreateHmac = crypto.createHmac as jest.MockedFunction<typeof crypto.createHmac>;
  ...
  describe('hashPassword', () => {
    ...
    it('should handle scrypt errors', async () => {
      ...
      await expect(service.hashPassword(password)).rejects.toThrow('Scrypt failed');
    });
  ...
  describe('comparePasswords', () => {
    it('should return true for matching passwords', async () => {
      ...
      expect(mockScrypt).toHaveBeenCalledWith(plainPassword, salt, 64, expect.any(Function));
    ...
    it('should return false for malformed hashed password', async () => {
      ...
      const result = await service.comparePasswords(plainPassword, malformedHash);
  ...
  describe('hashWithSalt', () => {
    it('should hash data with provided salt', async () => {
      ...
      expect(mockScrypt).toHaveBeenCalledWith(data, salt, 64, expect.any(Function));
    ...
    it('should handle scrypt errors in hashWithSalt', async () => {
      ...
      await expect(service.hashWithSalt('data', 'salt')).rejects.toThrow('Hash with salt failed');
    });
  });
  ...
  describe('hashResetToken', () => {
    it('should hash reset token with shorter salt', async () => {
      ...
      expect(mockScrypt).toHaveBeenCalledWith(token, mockSalt, 32, expect.any(Function)); // Shorter hash
      expect(result).toBe(`${mockSalt}:${mockHash.toString('hex')}`);
    });
    ...
    it('should handle scrypt errors in hashResetToken', async () => {
      ...
      await expect(service.hashResetToken('token')).rejects.toThrow('Reset token hash failed');
    });
  });
  ...
  describe('verifyResetToken', () => {
    it('should return true for valid reset token', async () => {
      ...
      expect(mockScrypt).toHaveBeenCalledWith(token, salt, 32, expect.any(Function));
      expect(result).toBe(true);
    });
  ...
  describe('Edge Cases and Security', () => {
    it('should handle null and undefined inputs gracefully', async () => {
      ...
      await expect(service.comparePasswords(null as any, 'hash')).resolves.toBe(false);
      await expect(service.comparePasswords('password', null as any)).resolves.toBe(false);
    });
    ...
    it('should handle very long strings', async () => {
      ...
      await expect(service.hashPassword(veryLongPassword)).resolves.toBeDefined();
    });
    ...
    it('should handle buffer allocation errors', async () => {
      ...
      await expect(service.hashPassword('password')).rejects.toThrow('Buffer allocation failed');
    });
