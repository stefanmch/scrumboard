...
describe('RolesGuard', () => {
  ...
  describe('canActivate', () => {
    ...
    it('should throw ForbiddenException when user does not have any of multiple required roles', () => {
      ...
      expect(() => guard.canActivate(mockExecutionContext)).toThrow(
        new ForbiddenException('Access denied. Required roles: ADMIN, SCRUM_MASTER')
      );
    });
  ...
  describe('Role Hierarchy Tests', () => {
    ...
    it('should work with all standard roles', () => {
      const standardRoles = ['ADMIN', 'SCRUM_MASTER', 'PRODUCT_OWNER', 'DEVELOPER', 'STAKEHOLDER', 'MEMBER'];
  ...
  describe('Error Message Tests', () => {
    ...
    it('should provide clear error message for multiple required roles', () => {
      reflector.getAllAndOverride.mockReturnValue(['ADMIN', 'SCRUM_MASTER', 'PRODUCT_OWNER']);
      mockRequest.user = {
        sub: '1',
        email: 'user@example.com',
        roles: ['MEMBER'],
      ...
      expect(() => guard.canActivate(mockExecutionContext)).toThrow(
        new ForbiddenException('Access denied. Required roles: ADMIN, SCRUM_MASTER, PRODUCT_OWNER')
      );
    });
  });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle roles with special characters and numbers', () => {
      const specialRoles = ['ROLE-WITH-DASHES', 'ROLE_WITH_UNDERSCORES', 'ROLE123', 'ROLE@SYMBOL'];
  ...
  describe('Performance Tests', () => {
    ...
    it('should short-circuit on first matching role', () => {
      reflector.getAllAndOverride.mockReturnValue(['FIRST_ROLE', 'SECOND_ROLE', 'THIRD_ROLE']);
      mockRequest.user = {
        sub: '1',
        email: 'user@example.com',
        roles: ['FIRST_ROLE'],
