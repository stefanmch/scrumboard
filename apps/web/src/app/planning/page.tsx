...
export default function PlanningPage() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      // Fetch first project if not already loaded
      let currentProjectId = projectId
      if (!currentProjectId) {
        // Get first team
        const teams = await teamsApi.getAll()
        if (teams.length === 0) {
          toast.showError('No teams found. Please create a team first.')
          setIsLoading(false)
          return
        }

        // Get first project from first team
        const projects = await projectsApi.getAllForTeam(teams[0].id)
        if (projects.length === 0) {
          toast.showError('No projects found. Please create a project first.')
          setIsLoading(false)
          return
        }
      ...
      const [sprintsData, storiesData] = await Promise.all([
        sprintsApi.getAll({ status: 'PLANNING', projectId: currentProjectId }),
        storiesApi.getAll(currentProjectId),
      ])

      setSprints(sprintsData)
      // Filter stories without sprint assignment
  ...
  const handleCreateSprint = async (data: Partial<Sprint>) => {
    ...
    try {
      ...
      if (error?.status === 409 || error?.message?.includes('overlap')) {
